...
public class FletcherReeves {
    ...
    public double[] minimize(double[] x0){
        ...
        powell = new Dichotomy(new FletcherReeves.LambdaFun());
    ...
    public FletcherReeves(MultipleVariablesFunction function) {
    ...
    class LambdaFun implements Function {
        ...
        public double f(double lambda) {
            double[] d0 = FletcherReeves.this.d.clone();
            double[] x0 = FletcherReeves.this.x.clone();
            for(int i=0; i<d0.length; i++){
                d0[i]*=lambda;
            }
            return FletcherReeves.this.f(sumOfVectors(x0,d0));
        }

        @Override
        public double dif(double lambda, int n) {
            double[] d0 = FletcherReeves.this.d.clone();
            double[] x = FletcherReeves.this.x.clone();
            for(int i=0; i<d0.length; i++){
                d0[i]*=lambda;
            }
            return FletcherReeves.this.f(sumOfVectors(x,d0));
